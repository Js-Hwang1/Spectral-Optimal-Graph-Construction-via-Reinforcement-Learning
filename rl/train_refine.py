#!/usr/bin/env python3
"""
REFINE v8 — Per-Swap Reward PPO Training.

Same architecture as v7 (Edge-MLP + RR subspace tracking, 10-dim features).
Key change: each swap (add + remove) gets its own exact Δλ₂ reward,
giving per-swap credit assignment instead of lumping M/10 swaps into one reward.

Training: O(N³) eigensolve per swap (acceptable at n=8-16).
Inference: Same batch-scored + row-update structure, no eigensolves.

Usage:
    python train_refine.py --min-n 8 --max-n 16 --episodes 20000 --swap-frac 0.1
"""
...
# ============================================================================
# Episode collection with PER-SWAP rewards
# ============================================================================

def collect_episode(env, policy, n, m, k_steps, device, swap_frac=0.1):
    """
    Collect one episode with per-swap exact Δλ₂ rewards.

    Each swap (add + remove) is one RL transition with its own reward.
    This gives per-swap credit assignment instead of lumping M/10 swaps
    into one macro reward.
    """
    env.reset(n=n, m=m)
    ...
    for k in range(k_steps):
        ...
        # Initial full scoring O(N²)
        edge_feat, _ = env.get_edge_level_features_rr()
        ef_t = torch.tensor(edge_feat, dtype=torch.float32, device=device)

        with torch.no_grad():
            add_logits = policy.score_add(ef_t).cpu().numpy()  # (N,N)
            rem_logits = policy.score_remove(ef_t).cpu().numpy()  # (N,N)
        ...
        # End of K-step: update env state
        env.lambda2 = lambda2_cache
        env.step += 1
...
def train_refine(
    ...
    """Train REFINE v8 with per-swap exact Δλ₂ rewards."""
    ...
    if save_dir is None:
        save_dir = Path("logs") / f"refine_v8_{timestamp}"
    else:
    ...
    print("=" * 70)
    print("REFINE v8 — Per-Swap Reward PPO")
    print("=" * 70)
    ...
    for update in pbar:
        ...
        for b in range(batch_size):
            ...
            (ep_values, ep_log_probs, ep_rewards,
             ep_entropies, ep_ctxs, ep_metrics) = collect_episode(
    ...
    print(f"\n{'='*70}")
    print("REFINE v8 (Per-Swap Reward) Training Complete!")
...
def main():
    parser = argparse.ArgumentParser(
        description="REFINE v8: Per-Swap Reward PPO")
    ...
    parser.add_argument("--swap-frac", type=float, default=0.1,
                        help="Fraction of edges to swap per step (default 0.1 = 10%%)")
